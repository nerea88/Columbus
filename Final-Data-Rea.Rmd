...
col_dorling@data <- merge( col_dorling@data, d, by.x="GEOID", by.y="tractid", all.x=T )
```


```{r}
# data frame and polygon ID standardization in case a tract was dropped and IDs don't match
row.ids <- sapply( slot( col_dorling, "polygons" ), function(x) slot( x, "ID" ) )
row.names( col_dorling ) <- row.ids

# project to standard lat-lon coordinate system 
col_dorling <- spTransform( col_dorling, CRS("+proj=longlat +datum=WGS84") )

# write to file 
geojson_write( col_dorling, file="columbus_dorling.geojson", geometry="polygon" )
```
